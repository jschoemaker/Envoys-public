...
reconstruct its value from the receiver's own identity — the authority
it knows itself to be serving (configuration) or the request's `Host`
header — never from any other sender-controlled field, and lowercase it
before use. A verifier that cannot determine its own authority MUST
reject signatures covering `@authority`.
