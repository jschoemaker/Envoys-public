...
The `content-digest` header MUST be a digest of the literal request body
bytes structured per RFC 9530. The digest algorithm is `sha-256` by
default. Senders MAY auto-promote to `sha-512` for bodies of 4096 bytes
or larger; receivers MUST accept both algorithms (§5.3).
